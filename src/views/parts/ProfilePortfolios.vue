    ...
    <div v-if="portfolios">
      ...
      >
        ...
        <b-button
          ...
          >Editar</b-button
        >
        <b-button variant="danger" size="sm" @click="remove(portfolio)"
          >Excluir</b-button
        >
      </b-card>
    </div>
  ...
  methods: {
    async refreshPortfolios() {
      const response = await apiProtected.get(
        `/users/${this.$route.params.userId}/portfolios`
      );
    ...
    async sendUpdate() {
      ...
      await apiProtected.patch(
        `/users/${this.$route.params.userId}/portfolios/${this.form.id}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
    ...
    async remove(portfolio) {
      try {
        await apiProtected.delete(
          `/users/${this.$route.params.userId}/portfolios/${portfolio.id}`
        );
        await this.refreshPortfolios()
      } catch (error) {
        console.error(error)
