...
export default {
  ...
  methods: {
    async onSubmit(data) {
      try {
        const response = await apiProtected.patch("applications/"+this.$route.params.applicationId, data);
  ...
  async mounted() {
    const response = await apiProtected(`applications/${this.$route.params.applicationId}`);
